...
def get_curve(img, leftx, rightx):
    ploty = np.linspace(0, img.shape[0]-1, img.shape[0])
    y_eval = np.max(ploty)
    ym_per_pix = 200/480 # centi-meters per pixel in y dimension
    xm_per_pix = 80/640 # centi-meters per pixel in x dimension
...
def get_steer_cmd(curverad):
    cmd = {"action":"2", "steerAngle":-curverad[2]}
